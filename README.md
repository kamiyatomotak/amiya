...
1年の進行度を計算し、毎日朝8時（日本時間）にX（旧Twitter）へ自動投稿するボットです。
...
*   上記を組み合わせた内容 (日付、経過日数/総日数/残り日数、プログレスバー、生成文) を、毎日朝8時 (JST) にXへ自動投稿します。

## 使い方 (GitHub Actionsによる自動実行)

このリポジトリでは、GitHub Actions を利用して、毎日自動でツイートが投稿されるように設定されています (`.github/workflows/cron_tweet.yml`)。
